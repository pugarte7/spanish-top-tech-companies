...
is 60k+ base. Read [docs/CONTEXT.md](docs/CONTEXT.md) before
...
The bar decides which companies are listed (median at the bar) and is shown
as the share; it never trims the rows the median is of. Filter positions by
...
became median >= 60k, dropping 33 companies (BBVA, Minsait, EPAM, Accenture,
eDreams...). Do not bring back the 60k-filtered mean, the any-row rule, or
juniors and other families in the median.
...
A company is on the list only while its median base is at 60k+. There is no
...
`fetch_spain.write()` removes a company whose median comes back under the bar
unless a first-hand entry vouches for it, and `validate.py` fails on lasting
rows with a median under it. New companies arrive by discovery: each signed-in run reads the
