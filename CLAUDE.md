...
the median over every such engineer on file, at companies where at least one
of them is at 60k+ base. Read [docs/CONTEXT.md](docs/CONTEXT.md) before
...
The bar decides which companies are listed (at least one row at the bar) and
is measured by the share; it never trims the rows the median is of. Seniority
...
nothing). Do not bring back the 60k-filtered mean, and do not put juniors or
other families into the median.
...
A company is on the list only while one of its rows is at 60k+. There is no
...
`fetch_spain.write()` removes a company that comes back with nobody at the bar
unless a first-hand entry vouches for it, and `validate.py` fails on lasting
rows with nobody at the bar. New companies arrive by discovery: each signed-in run reads the
