...
A company with no Spanish pay still gets a file and a row. `spain_check` records
which job families were asked and what came back, and the front page renders it
as `no Spain data` rather than a dash, so "asked, nothing published" never reads
as "nobody has looked". Never infer that state from the wording of a note.
